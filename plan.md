...
- 进展记录（2026-04-20 18:20）
  - 已为 `mars-moon` 补充根目录 `.gitignore`，当前默认忽略 `.venv/`、`tmp/`、`result/`、`work/*/`、日志与临时文件，避免单次调试结果和运行产物混入 skill 仓库。
  - 已补充结果归档说明：`references/workflow.md` 新增“源码与结果分离 / 结果怎么归档 / 发布到 GitHub 前的建议”三部分；`SKILL.md` 同步补充了 Git 发布和结果保留口径。
  - 已重新通过 `skill-git` 将更新同步到远端 `zjashanda/mars-moon`，最新提交为 `bb2ef3d Publish mars-moon`，远端 `origin/main` 已同步到该提交。

- 进展记录（2026-04-20 18:31）
  - 已完成本地目录清理：删除了 `work/` 下所有单次运行目录、`tmp/` 历史输入/调试产物、`result/` 历史结果目录，以及各处 `__pycache__`；保留 `work/` 根目录下的长期分析文档与规格化中间文件。清理后本地占用约为：`.venv 32K`、`tmp 4K`、`result 4K`、`work 192K`。
  - 当前本地刻意保留 `.venv/`，未做删除；若后续需要可再单独清理。
  - 已复核云端仓库内容：此前远端主要多余项为 `plan.md` 这类本机执行态文件。现已通过重新发布干净副本将其移除，远端仅保留 skill 主体源码、文档、资源和生成基线。
  - 最新远端提交为 `b192b6d Publish mars-moon`，`origin/main` 已同步到该提交。

- 进展记录（2026-04-23 00:00）
  - 已按当前目录约定重新读取 `plan.md` 与 `SKILL.md`，本轮先向用户说明 `mars-moon` 当前主要负责的验证范围与执行闭环，不涉及新的构建、烧录或实机运行。

- 进展记录（2026-04-23 00:05）
  - 本轮任务为方法说明：基于 `mars-moon` 当前能力，提炼“拿到一份新需求表后，如何对比功能点并开始验证对应需求”的执行思路，输出仅限思路与方法，不做代码修改和实机执行。

- 进展记录（2026-04-23 00:12）
  - 已新增汇总方法文档 `references/需求功能验证设计方法.md`，将“拿到新需求表后的分析模板 + 功能点到验证方案设计逻辑 + 结果记录与归因模板”三部分收敛到一份文档中。
  - 本次重点已按要求展开第二部分：更详细说明了功能点如何拆正例/负例/异常场景、如何区分前置与主体、如何定义必要证据/辅助证据，以及如何按功能类型套用验证设计模式。


- 进展记录（2026-04-23 00:18）
  - 已按用户要求把 `references/test-plan-draft.md` 与 `references/需求功能验证设计方法.md` 融合为单一主文档。
  - 当前保留文件为 `references/需求功能验证设计方法.md`，原 `references/test-plan-draft.md` 已删除。
  - 已复核主文档章节结构与遗留引用：章节编号已连续调整到 `1~8`，仓内已无对 `test-plan-draft.md` 的残留引用。
  - 下一步仅向用户展示融合后的主文件结构与主要内容，不进行设备执行或 runner 改动。

- 进展记录（2026-04-23 00:24）
  - 已按用户补充要求增强 `references/需求功能验证设计方法.md`，新增“正式测试前的最小可测性验证”门禁逻辑。
  - 当前已明确写入两条前置硬规则：
    1. 正式测试前必须确认设备可正常唤醒、识别基础命令词、输出对应日志和响应播报，且设备不存在反复重启；
    2. 若现场存在独立协议串口，主动/被动协议校验均优先以协议串口为主证据，日志中的 `send msg` / `recv msg` 仅作辅助印证。
  - 已同步更新文档中的总体流程、证据优先级、UART 章节和最终落地建议；下一步向用户展示修改后的内容，不进行设备执行。


- 进展记录（2026-04-23 00:36）
  - 已按用户补充要求继续增强 `references/需求功能验证设计方法.md`，新增烧录门禁与烧录后健康检查章节。
  - 当前已写入的烧录口径包括：区分 Linux/Windows 烧录环境、烧录前删除旧 `app.bin` 并将新固件统一命名为 `app.bin`、进入烧录模式必须完整执行“断电 -> 进 boot -> 上电 -> 下 boot”、任一步失败都必须整套重来、烧录完成后需连日志口上电观察 `20s`，若出现自动重启则立即停测上报。
  - 已明确最终门禁顺序：烧录后健康检查通过后，还必须继续执行最小可测性验证；若最小可测性验证不通过，同样停止测试并上报异常。
  - 已新建组合示例文档 `references/masr-pillar.md`，将当前项目需求提炼、补充逻辑、烧录门禁、最小可测性验证、协议优先级、模块级测试方案、停测规则收敛到一份“需求-测试方案示例文档”，供后续其他 skill 读取复用。


- 进展记录（2026-04-23 00:43）
  - 已按用户要求结合历史测试数据重新梳理“当前需求验证仍异常的功能点”。
  - 参考口径以 `work/requirement-status-20260409.md` 的主需求状态、`references/current-baseline.md` 的最新 bundle 基线，以及 `work/analysis-20260409-requirement-case-gap.md` 的归因分析为主。
  - 当前结论已收敛为三层：
    1. 主需求里明确仍异常的 `FAIL` 项；
    2. 主需求里仍需补强验证的 `PARTIAL / NO_METHOD` 项；
    3. 不建议与主需求混算、但历史测试中仍存在差异的衍生项（如欢迎语、配对遥控器入口协议）。


- 进展记录（2026-04-23 14:12）
  - 已按用户要求将当前修改同步到远端 Git 仓库 `zjashanda/mars-moon`。
  - 本次先通过本地发布缓存仓库 `~/.codex/skill-git-repos/mars-moon` 同步当前 skill 内容并提交，再直接使用 SSH `push` 完成发布。
  - 远端已更新到提交：`2af73a0 Update validation methodology and masr-pillar docs`。
  - 本次同步的核心内容包括：删除 `references/test-plan-draft.md`、更新 `references/需求功能验证设计方法.md`、新增 `references/masr-pillar.md`，以及补充当前保留的 `work/*.md/*.json` 分析基线文件。

- 进展记录（2026-04-23 14:20）
  - 已继续根据新写入的 `references/` 文档反向补充当前 skill 主文档，重点同步到 `SKILL.md` 与 `references/workflow.md`。
  - 当前已补回的核心口径包括：
    1. 启动后优先阅读 `references/需求功能验证设计方法.md` 与 `references/masr-pillar.md`；
    2. 烧录前固件需复制到实际烧录目录，删除旧 `app.bin` 后再统一命名为 `app.bin`；
    3. 进入烧录模式必须完整执行“断电 -> 进 boot -> 上电 -> 下 boot”，任一步失败都要整套重来；
    4. 烧录完成后必须做 `20s` 健康检查；
    5. 正式测试前必须做最小可测性验证；
    6. 有独立协议串口时，主动/被动协议都优先以协议串口为主断言来源。
  - 同时已把新的方法文档、示例文档和历史分析基线文档加入 `SKILL.md` 的资源索引，便于后续其他 agent/skill 接手时直接定位。

- 进展记录（2026-04-23 14:27）
  - 已按当前轮需求只执行“增强项 1”，在 `references/workflow.md` 中新增“结果判读速查清单”。
  - 当前速查清单已覆盖：
    1. 什么算功能 `PASS`；
    2. 什么算功能 `FAIL`；
    3. 什么属于“功能通过但数值有差异”； 
